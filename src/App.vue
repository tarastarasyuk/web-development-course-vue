    ...
    <section>
      <div class="flex">
        <div class="max-w-xs">
          ...
          <div v-if="!isValidTickerName" class="text-sm text-red-600">This ticker has already been added</div>
        </div>
      </div>
      <button
        @click="add()"
...
export default {
  ...
  methods: {
    add() {
      ...
      setInterval(async() => {
        const f = await fetch(
          `https://min-api.cryptocompare.com/data/price?fsym=${currentTicker.name}&tsyms=USD&api_key=4caa95b1312b4ddfca598bf879d242dd3c2c8cc635c161e4e515758bd82717e9`
          );
        const data = await f.json();
